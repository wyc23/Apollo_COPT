...
def run_parallel(func, args_list, n_jobs=None):
    if n_jobs is None:
...
def generate_setcover_with_seed(nrows, ncols, dens, filename, seed, max_coef):
    rng = np.random.RandomState(seed)
    return generate_setcover(nrows, ncols, dens, filename, rng=rng, max_coef=max_coef)

def generate_cauctions_with_seed(rng_seed, filename, n_items, n_bids):
    rng = np.random.RandomState(rng_seed)
    return generate_cauctions(rng, filename, n_items=n_items, n_bids=n_bids, add_item_prob=0.7)
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        choices=['sc', 'ca', 'facilities', 'indset'],
    )
    parser.add_argument(
        '-s', '--seed',
    ...
    if args.problem == 'sc':
        ...
        lp_dir = f'data/instances/sc/train'
        ...
        lp_dir = f'data/instances/sc/valid'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nrowss.extend([nrows] * n)
        ...
        lp_dir = f'data/instances/sc/test'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nrowss.extend([nrows] * n)
        ...
        for filename, nrows, ncols, dens in zip(filenames, nrowss, ncolss, denss):
            ...
            args_list.append((nrows, ncols, dens, filename, seed, max_coef))

        run_parallel(generate_setcover_with_seed, args_list, n_jobs=args.threads)
        ...
        lp_dir = f'data/instances/ca/train'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        ...
        lp_dir = f'data/instances/ca/valid'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nitemss.extend([number_of_items] * n)
        ...
        lp_dir = f'data/instances/ca/test'
        print(f"{n} instances in {lp_dir}")
        os.makedirs(lp_dir)
        filenames.extend([os.path.join(lp_dir, f'instance_{i+1}.lp') for i in range(n)])
        nitemss.extend([number_of_items] * n)
        ...
        for filename, nitems, nbids in zip(filenames, nitemss, nbidss):
            seed = master_rng.randint(1e9)
            args_list.append((seed, filename, nitems, nbids))

        run_parallel(generate_cauctions_with_seed, args_list, n_jobs=args.threads)
